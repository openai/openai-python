...
| CI and publication | Test PR and branch code, build distributions, and publish through protected release paths. | [.github/workflows/ci.yml:18](../../.github/workflows/ci.yml), [.github/workflows/publish-pypi.yml:8](../../.github/workflows/publish-pypi.yml) |
...
flowchart LR
  ...
  pr[PR checkout code] -->|read-only CI execution| ci[CI runner]
...
security decisions; or PR-controlled code reaching protected CI/release
credentials or publication authority. Ordinary authorized SDK behavior,
...
- **High:** disclosure of a high-privilege admin, provider, or release
  credential; durable cross-account authority; or a protected credential-
...
- **Medium:** forged webhook acceptance, host-identity token abuse, or remote
  parser/stream availability impact with a realistic independently mutable
  input and deployment prerequisite
  ([src/openai/lib/_webhooks.py:20](../../src/openai/lib/_webhooks.py),
