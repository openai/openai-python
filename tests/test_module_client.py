...
# Reset OpenAI client state before each test
...
# Test setting and getting the base URL option
...
# Test setting and getting the timeout option
def test_timeout_option() -> None:
    assert openai.timeout == DEFAULT_TIMEOUT
    assert openai.completions._client.timeout == DEFAULT_TIMEOUT
    openai.timeout = 3
    assert openai.timeout == 3
    assert openai.completions._client.timeout == 3

# Test setting and getting the max retries option
def test_max_retries_option() -> None:
    assert openai.max_retries == DEFAULT_MAX_RETRIES
    assert openai.completions._client.max_retries == DEFAULT_MAX_RETRIES
    openai.max_retries = 1
    assert openai.max_retries == 1
    assert openai.completions._client.max_retries == 1

# Test setting and getting the default headers option
...
# Test setting and getting the default query option
...
# Test setting and getting the HTTP client option
...
# Context manager for temporarily modifying the environment
...
# Test OpenAI API type when only API key is provided
...
# Test handling of Azure API key without API version
def test_azure_api_key_env_without_api_version() -> None:
    with fresh_env():
        openai.api_type = None
        _os.environ["AZURE_OPENAI_API_KEY"] = "example API key"
        with pytest.raises(ValueError, match=r"Must provide either the `api_version` argument or the `OPENAI_API_VERSION` environment variable"):
            openai.completions._client

# Test handling of Azure API key, version, and endpoint in environment variables
...
# Test handling of Azure API key, Azure AD token, version, and endpoint in environment variables
...
# Test handling of Azure API key, Azure AD token provider, version, and endpoint in environment variables
