...
class TestOpenAI:
    ...
    @pytest.mark.respx2(base_url=base_url)
    def test_429_insufficient_quota(self, respx2_mock: MockRouter) -> None:
        respx2_mock.post(base_url + "/chat/completions").mock(
            return_value=httpx2.Response(
                429,
                json={
    ...
    @pytest.mark.respx2(base_url=base_url)
    def test_429_rate_limit_without_insufficient_quota(self, respx2_mock: MockRouter) -> None:
        respx2_mock.post(base_url + "/chat/completions").mock(
            return_value=httpx2.Response(
                429,
                json={
    ...
    @mock.patch("openai._base_client.BaseClient._calculate_retry_timeout", _low_retry_timeout)
    @pytest.mark.respx2(base_url=base_url)
    def test_429_insufficient_quota_is_not_retried(self, respx2_mock: MockRouter) -> None:
        respx2_mock.post(base_url + "/chat/completions").mock(
            return_value=httpx2.Response(
                429,
                json={
    ...
    @mock.patch("openai._base_client.BaseClient._calculate_retry_timeout", _low_retry_timeout)
    @pytest.mark.respx2(base_url=base_url)
    def test_429_insufficient_quota_is_not_retried_streaming(self, respx2_mock: MockRouter) -> None:
        respx2_mock.post(base_url + "/chat/completions").mock(
            return_value=httpx2.Response(
                429,
                json={
        ...
        with OpenAI(
            ...
            with pytest.raises(InsufficientQuotaError):
                with client.chat.completions.with_streaming_response.create(messages=[], model="gpt-4") as response:
                    response.read()

            # The error body is only available once the stream is read, but the
            # `insufficient_quota` classification must still apply and the request
            # should not be retried even though max_retries > 0.
            calls = cast("list[MockRequestCall]", respx2_mock.calls)
            assert len(calls) == 1

    @mock.patch("openai._base_client.BaseClient._calculate_retry_timeout", _low_retry_timeout)
    @pytest.mark.respx2(base_url=base_url)
    def test_429_still_retried_if_streamed_body_read_fails(self, respx2_mock: MockRouter, client: OpenAI) -> None:
        respx2_mock.post("/chat/completions").mock(return_value=httpx2.Response(429))

        # respx2 pre-reads its own canned response while resolving each mocked call
        ...
        original_read = httpx2.Response.read
        read_calls = [0]

        def flaky_read(self: httpx2.Response) -> bytes:
            read_calls[0] += 1
            if read_calls[0] % 2 == 0:
                raise httpx2.ReadError("mid-stream connection reset")
            return original_read(self)

        with mock.patch.object(httpx2.Response, "read", flaky_read):
...
class TestAsyncOpenAI:
    ...
    @pytest.mark.respx2(base_url=base_url)
    async def test_429_insufficient_quota(self, respx2_mock: MockRouter) -> None:
        respx2_mock.post(base_url + "/chat/completions").mock(
            return_value=httpx2.Response(
                429,
                json={
    ...
    @pytest.mark.respx2(base_url=base_url)
    async def test_429_rate_limit_without_insufficient_quota(self, respx2_mock: MockRouter) -> None:
        respx2_mock.post(base_url + "/chat/completions").mock(
            return_value=httpx2.Response(
                429,
                json={
    ...
    @mock.patch("openai._base_client.BaseClient._calculate_retry_timeout", _low_retry_timeout)
    @pytest.mark.respx2(base_url=base_url)
    async def test_429_insufficient_quota_is_not_retried(self, respx2_mock: MockRouter) -> None:
        respx2_mock.post(base_url + "/chat/completions").mock(
            return_value=httpx2.Response(
                429,
                json={
    ...
    async def test_429_still_retried_if_streamed_body_read_fails(
        ...
        # Fail that one to simulate the connection dropping mid-stream; that must not
        # stop a plain 429 from being retried on status code alone.
        original_aread = httpx2.Response.aread
        aread_calls = [0]

        async def flaky_aread(self: httpx2.Response) -> bytes:
            aread_calls[0] += 1
            if aread_calls[0] % 2 == 0:
                raise httpx2.ReadError("mid-stream connection reset")
            return await original_aread(self)

        with mock.patch.object(httpx2.Response, "aread", flaky_aread):
            with pytest.raises(RateLimitError):
                await async_client.chat.completions.with_streaming_response.create(
                    messages=[
                        {
    ...
    @mock.patch("openai._base_client.BaseClient._calculate_retry_timeout", _low_retry_timeout)
    @pytest.mark.respx2(base_url=base_url)
    async def test_429_insufficient_quota_is_not_retried_streaming(self, respx2_mock: MockRouter) -> None:
        respx2_mock.post(base_url + "/chat/completions").mock(
            return_value=httpx2.Response(
                429,
                json={
